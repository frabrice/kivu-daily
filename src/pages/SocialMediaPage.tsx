...
import { Plus, ChevronLeft, ChevronRight, Rocket, Image as ImageIcon, Zap, Trash2, ListFilter, CalendarDays } from 'lucide-react';
...
export default function SocialMediaPage() {
  ...
  const [editPost, setEditPost] = useState<ContentPost | 'new' | null>(null);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2.5">
        <div className="flex gap-0.5 p-0.5 bg-gray-100 dark:bg-white/5 rounded-lg w-fit">
          <TabButton active={view === 'week'} onClick={() => setView('week')} icon={CalendarDays} label="Week" />
          <TabButton active={view === 'list'} onClick={() => setView('list')} icon={ListFilter} label="List" />
        </div>
            ...
            return (
              <div key={ds} className="space-y-2">
                ...
                <div className="space-y-1.5 min-h-[50px]">
                  {dayPosts.map((p) => (
                    <button key={p.id} onClick={() => setEditPost(p)} className="w-full card p-2.5 text-left hover:shadow-md hover:border-brand/30 transition-all">
      ...
      {!loading && view === 'list' && (
        <div className="space-y-1.5">
          ...
          {posts.map((p) => (
            <button key={p.id} onClick={() => setEditPost(p)} className="w-full card p-3 flex items-center gap-3 text-left hover:shadow-md hover:border-brand/30 transition-all">
...
function PostDrawer({
  ...
  return (
    <Modal open onClose={onClose} title={post ? (post.pillar ?? 'Edit Post') : 'New Post'} subtitle="Plan the content, not just the date" maxWidth="max-w-lg">
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className="block text-[12px] font-medium mb-1.5 text-gray-500">Date</label>
            <input type="date" value={postDate} onChange={(e) => setPostDate(e.target.value)} className="input" />
          </div>
          <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {PLATFORMS.map((p) => (
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${platforms.includes(p) ? 'border-brand bg-brand/10 text-brand-700 dark:text-brand-300' : 'border-gray-200 dark:border-white/10 text-gray-500'}`}
              >
                {p}
              </button>
            ))}
        ...
        <div>
          <label className="flex items-center gap-2 text-[12px] font-medium text-gray-500 mb-1.5">
            <input type="checkbox" checked={boosted} onChange={(e) => setBoosted(e.target.checked)} className="rounded" />
            Boosted post
          </label>
          {boosted && (
            <input type="number" value={budget} onChange={(e) => setBudget(e.target.value)} placeholder="Budget (RWF)" className="input" />
          )}
        </div>

        <div>
          <label className="block text-[12px] font-medium mb-1.5 text-gray-500">Caption</label>
          <textarea value={caption} onChange={(e) => setCaption(e.target.value)} rows={3} className="input resize-none" placeholder="Mwaramutse! Beyond Transport. Into the Future." />
        </div>

        {status === 'posted' && (
          <div>
            <label className="block text-[12px] font-medium mb-1.5 text-gray-500">Performance notes</label>
            <textarea value={performanceNotes} onChange={(e) => setPerformanceNotes(e.target.value)} rows={2} className="input resize-none" placeholder="Reach, engagement, saves…" />
          </div>
        )}
