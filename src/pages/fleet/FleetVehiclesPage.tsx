  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2.5">
        <div>
          <h2 className="text-base font-semibold flex items-center gap-2"><Car size={16} className="text-blue-600 dark:text-blue-300" /> Vehicles</h2>
          <p className="text-[11px] text-gray-400 mt-0.5">Plate, RURA license, device, and documents handed over with each car.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <p className="text-[12px] font-semibold">{v.plate_number}</p>
                  <EntryActions
                ...
                <div className="flex flex-wrap gap-1.5 mb-2.5">
                  ...
                  {v.device_label && (
                    <span className="inline-flex items-center gap-1 text-[9px] font-medium text-brand-700 dark:text-brand-300 bg-brand/10 px-1.5 py-0.5 rounded-full">
                      <Smartphone size={9} /> {v.device_label}
                    </span>
                  )}
                  {v.documents.map((doc, i) => (
                    <span key={i} className="inline-flex items-center gap-1 text-[9px] font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded-full">
          ...
          {filteredVehicles.length === 0 && (
            <div className="card p-10 text-center col-span-full">
              ...
              <p className="text-[12px] text-gray-400">No vehicles yet.</p>
            </div>
          )}
        </div>
      )}
