...
export default function FinanceAccountsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-base font-semibold flex items-center gap-2"><Landmark size={16} className="text-amber-600 dark:text-amber-300" /> Bank Accounts</h2>
        <p className="text-[11px] text-gray-400 mt-0.5">The three principal accounts plus MoMo, each with a defined purpose. Balances update live from the ledger.</p>
      </div>
          ...
          <div key={a.id} className="card p-4">
            <div className="flex items-start justify-between gap-2 mb-1.5">
              <div>
                ...
                <p className="text-[10px] text-gray-400">{a.bank_name}</p>
              </div>
            ...
            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-2">{a.purpose}</p>
          </div>
        ))}
        {accounts.length === 0 && (
          <div className="card p-10 text-center col-span-full">
            ...
            <p className="text-[12px] text-gray-400">No accounts set up yet.</p>
          </div>
        )}
      </div>
