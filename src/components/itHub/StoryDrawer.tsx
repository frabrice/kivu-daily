    ...
    <Modal open onClose={onClose} title={story ? (story.source === 'flagged' ? 'Issue' : 'User Story') : (isIssue ? 'New Issue' : 'New User Story')} maxWidth="max-w-lg">
      {!editing && story ? (
        <div className="space-y-4">
          <div className="card p-3.5 bg-gray-50 dark:bg-white/5 text-[13px] leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {statusMeta && (
              <span className="text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: `${statusMeta.color}20`, color: statusMeta.color }}>
                {statusMeta.label}
              </span>
            )}
            <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full ${PRIORITY_STYLE[priority]}`}>
              <Flag size={10} /> {priority}
            </span>
            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded-full">
              <User size={10} /> {itProfiles.find((p) => p.id === assigneeId)?.full_name ?? 'Unassigned'}
            </span>
          </div>

          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-1.5">Details</p>
          ...
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-1.5 flex items-center gap-1.5">
              <ListChecks size={12} /> Acceptance Criteria
            </p>
            {criteria.length === 0 ? (
              <p className="text-[13px] text-gray-400">No acceptance criteria yet.</p>
            ) : (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-1.5">
              ...
              {!editing && criteria.length === 0 && <p className="text-[12px] text-gray-400">No acceptance criteria yet.</p>}
            </div>
          </div>
