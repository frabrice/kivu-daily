      ...
      <div className="space-y-3">
        {logs.length === 0 ? (
          <div className="py-6 text-center">
            ...
            <p className="text-[12px] text-gray-400">No calls logged with this driver yet.</p>
          </div>
        ) : (
            ...
            {logs.map((l) => (
              <div key={l.id} className="p-2.5 rounded-lg border border-gray-100 dark:border-white/5">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-medium">{l.reason?.label ?? 'Call'}</p>
                  <span className="text-[9px] text-gray-400 shrink-0">{timeAgo(l.created_at)}</span>
                </div>
                <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">
                  {l.outcome?.label ?? 'No outcome'} · by {l.caller?.full_name ?? 'Unknown'}
                </p>
                {l.note && <p className="text-[11px] text-gray-600 dark:text-gray-300 mt-1.5">{l.note}</p>}
              </div>
            ))}
          </div>
        )}
